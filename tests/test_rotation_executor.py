...
def test_mutation_timeout_is_never_retried():
    ...
    g._limiter = Mock()
    g._nonce = Mock(); g._nonce.next.return_value = 123
    g._request = Mock(side_effect=TransientBrokerError("response lost"))
    g._max_retries = 3; g._sleep = Mock(); g._audit = None
...
def test_same_key_serializes_nonce_through_response(tmp_path, monkeypatch):
    ...
    def request(*a, **k):
        with lock:
            active.append(1)
            if len(active) > 1: overlap.append(True)
        time.sleep(.04)
...
from dublin_bot.models import Signal, Action
import dublin_bot.rotation_execution as execution

class Exchange:
    def __init__(self):
        self.orders = {}; self.trades = {}; self.ledger = {
            "DEPOSIT":dict(refid="deposit",type="deposit",asset="ZUSD",amount="100",fee="0")
        }; self.sent = []
        self.balance = Decimal("100"); self.base = Decimal("0")
        self.fail_submit = False; self.partial = False; self.race = Decimal("0")
    ...
    def fill(self, oid, qty, price=100):
        o = self.orders[oid]; qty = Decimal(str(qty)); price = Decimal(str(price))
        tid = "T" + str(len(self.trades)); cost = qty * price; fee = cost / 100
        side = o["descr"]["type"]
        ...
        o["trades"].append(tid)
        for k,v in [("vol_exec",qty),("cost",cost),("fee",fee)]: o[k]=str(Decimal(o[k])+v)
        ...
        if Decimal(o["vol_exec"]) == Decimal(o["vol"]): o["status"]="closed"
    ...
    def _private(self, endpoint, params=None):
        params = params or {}
        if endpoint == "TradeBalance": return {"eb":str(self.balance+self.base*100)}
        if endpoint == "BalanceEx": return {"ZUSD": {"balance":str(self.balance), "hold_trade":"0"}, "XXBT":{"balance":str(self.base),"hold_trade":"0"}}
        if endpoint == "OpenOrders": return {"open": {k:v for k,v in self.orders.items() if v["status"]=="open"}}
        if endpoint == "ClosedOrders":
            rows={k:v for k,v in self.orders.items() if v["status"]!="open"}
            return {"closed":rows,"count":len(rows)}
        if endpoint == "TradesHistory": return {"trades":self.trades,"count":len(self.trades)}
        if endpoint == "Ledgers": return {"ledger":self.ledger,"count":len(self.ledger)}
        if endpoint == "QueryTrades": return {t:self.trades[t] for t in params["txid"].split(",")}
        if endpoint == "QueryOrders": return {params["txid"]:self.orders[params["txid"]]}
        if endpoint == "AddOrder":
            self.sent.append(dict(params))
            if self.fail_submit: raise TimeoutError("accepted but response lost")
            oid = "O"+str(len(self.sent)); qty=Decimal(params["volume"])
...
def test_full_buy_owned_sell_and_restart(tmp_path):
    ...
    gw=Exchange(); s=config(tmp_path); path=tmp_path/"live.json"
...
def test_partial_entry_multiple_children_cancel_race_exact_remaining(tmp_path):
    gw=Exchange(); gw.partial=True; ex=execution.RotationExecutor(gw,config(tmp_path),tmp_path/"live.json")
...
def test_stop_fill_reconciles_without_strategy_sell_and_no_double_count(tmp_path):
    gw=Exchange(); ex=execution.RotationExecutor(gw,config(tmp_path),tmp_path/"live.json")
    assert ex.buy("BTC/USD",Signal(Action.BUY,80,"entry",price=100,stop_price=96))["executed"]
    gw.fill("O1S",".25",90)
    ex.reconcile(); once=ex.snapshot()
    ex.reconcile()
...
def test_lost_submit_response_restart_never_duplicates(tmp_path):
    gw=Exchange(); gw.fail_submit=True; path=tmp_path/"live.json"
...
def test_entry_evidence_failure_never_uses_fallback(tmp_path,fault):
    gw=Exchange(); original=gw._private
    def read(endpoint,params=None):
        if endpoint==fault: raise ValueError("authoritative read failed")
...
def test_incomplete_history_never_means_empty():
    g=Mock();g._private.return_value={"trades":{},"count":5}
...
def test_historical_migration_preview_checks_ledger_balance_and_writes_nothing(tmp_path):
    ...
    gw=Exchange(); gw.order("OKIUZG-6I4Y7-WFYFFZ","buy",".25"); gw.fill("OKIUZG-6I4Y7-WFYFFZ",".25")
    gw.ledger["DEPOSIT"]={"refid":"deposit","type":"deposit","asset":"ZUSD","amount":"100","fee":"0"}
    path=tmp_path/"live.json"; ex=execution.RotationExecutor(gw,config(tmp_path),path)
...
def test_cancel_response_loss_reconciles_final_child_before_sell(tmp_path):
    ...
    def lost(endpoint, params=None):
        ...
        if endpoint=="CancelOrder": raise TimeoutError("cancel response lost")
        return result
    gw._private=lost;gw.race=Decimal(".025")
    result=ex.sell("BTC/USD")
    assert result["executed"],result
    assert Decimal(gw.sent[-1]["volume"])==Decimal(".225")
...
def test_same_key_highwater_survives_different_local_nonce_files(tmp_path,monkeypatch):
    ...
    for candidate in (9999999999999999999,1):
        g=KrakenGateway.__new__(KrakenGateway)
        g._api_key="shared-watermark-test";g._api_secret=base64.b64encode(b"secret").decode()
        g._limiter=Mock();g._nonce=Mock();g._nonce.next.return_value=candidate
        g._max_retries=1
        def request(*a,**kw): values.append(int(kw["data"]["nonce"]));return {}
        g._request=request;g._private("Balance")
...
def _process_private_call(home, events, key, nonce_file):
    ...
    g=KrakenGateway.__new__(KrakenGateway);g._api_key=key
    g._api_secret=base64.b64encode(b"secret").decode()
    g._limiter=Mock();g._nonce=NonceGenerator(Path(nonce_file));g._max_retries=1
    def request(*a,**kw):
        with open(events,"a") as f: f.write("start "+kw["data"]["nonce"]+"\n")
        time.sleep(.1)
        with open(events,"a") as f: f.write("end\n")
        return {}
    g._request=request;g._private("Balance")
...
def test_private_serialization_across_processes_preserves_ledgers(tmp_path):
    ...
    for job in jobs: job.join(10);assert job.exitcode==0
...
def test_pending_partial_exit_blocks_duplicate_until_final_fill(tmp_path):
    ...
    def partial_sell(endpoint,params=None):
        if endpoint=="AddOrder" and params["type"]=="sell":
            gw.sent.append(dict(params));gw.order("O2","sell",params["volume"])
            gw.fill("O2",".1",110);return {"txid":["O2"]}
        return original(endpoint,params)
    ...
    gw.fill("O2",".15",110);ex.reconcile()
    assert ex.holdings()=={}
...
def test_live_position_info_never_uses_fallback_account_wrappers(tmp_path):
    from dublin_bot.agents.trading_agent import TradingAgent
    gw=Exchange(); agent=TradingAgent(config(tmp_path),gateway=gw)
...
def test_corrupt_owned_quantity_cannot_be_treated_as_flat(tmp_path):
    import json
    gw=Exchange(); path=tmp_path/"live.json";ex=execution.RotationExecutor(gw,config(tmp_path),path)
    ex.buy("BTC/USD",Signal(Action.BUY,80,"entry",price=100,stop_price=96))
    state=json.loads(path.read_text());state["lots"]["BTC/USD"]["qty"]="-1"
...
def test_exact_cost_comparison_does_not_roundtrip_average_price(tmp_path):
    gw=Exchange();ex=execution.RotationExecutor(gw,config(tmp_path),tmp_path/"live.json")
    gw.order("O1","buy",".03");gw.fill("O1",".03")
...
def test_unattributed_same_asset_sale_blocks_owned_exit_before_cancel(tmp_path):
    ...
    gw.order("MANUAL","sell",".1");gw.fill("MANUAL",".1")
    result=ex.sell("BTC/USD")
...
def test_accepted_response_loss_recovers_unique_exact_intent_no_resubmit(tmp_path):
    ...
    def lost(endpoint,params=None):
        ...
        return response
    gw._private=lost;ex=execution.RotationExecutor(gw,config(tmp_path),tmp_path/"live.json")
    result=ex.buy("BTC/USD",Signal(Action.BUY,80,"entry",price=100,stop_price=96))
...
def test_restart_resumes_reserved_exit_after_child_cancel_before_sell(tmp_path):
    from dublin_bot.agents.trading_agent import TradingAgent
    gw=Exchange();agent=TradingAgent(config(tmp_path),gateway=gw);agent.strategy._candidate="BTC/USD"
